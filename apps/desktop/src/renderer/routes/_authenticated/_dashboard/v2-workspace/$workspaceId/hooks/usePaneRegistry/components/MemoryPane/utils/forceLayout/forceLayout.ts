 ...
 * SVG graph view — no graph-lib dependency (A12) and no `@superset/memory`
 * runtime dep on the renderer (it isn't a desktop dependency; the renderer only
 * sees memory types via the host router output). Seeded initial placement +
 * fixed iteration count so the same graph lays out identically (no jitter,
 * snapshot-friendly). Not a physics sim; just enough to spread nodes legibly.
 */
...
export function computeForceLayout(
	...
	const adjacency = graph.edges.map((e) => ({ a: e.source, b: e.target }));
