...
- [x] B3 (2026-06-23) — DONE (both halves). B3-host: `approved_ticket_context` store (unique `(projectId,taskId)`, migration `0008`) + `ticketContext` router (`getApproved`/`saveApproved` with redact-on-save = redaction point #1), commit `3e059faf1`. B3-renderer: editable draft → "Approve context" (`saveApproved({projectId: primary, taskId, content})`), reopen shows the approved content (authoritative), the B2 regenerate-clobbers-edits coupling fixed (separate editableContent + explicit warned regenerate), and "Start autonomous run" gated until an approved context exists (one shared approval state keyed `(primaryProjectId, taskId)`). 36/36 tasks tests; typecheck + biome clean. Commit `cc15d3bdf`.
- [x] B4 (2026-06-23) — DONE. Default single repo (preselected via the run-in-workspace project picker); optional additional repos with EXACTLY ONE multi-repo confirm; "Start autonomous run" calls host `ticketRun.start` (args match `startInputSchema` byte-for-byte — parallel fixed-contract build, no reconciliation). Exposes primary `approvedContextProjectId` for B3-renderer. No worktrees pre-created. desktop typecheck + biome clean; tasks 29/29. Commit `1cfd5ab48`.
- [x] B5 (2026-06-23) — DONE. Host `ticketRun.start` orchestrator: reads approved context (throws PRECONDITION_FAILED if none), assembles layered prompt global<project<approved-AUTHORITATIVE-last + redacts (point #2) + a hard never-merge stop instruction; per-repo `workspaces.create({agents:[{agent:"claude",prompt}]})` with a `ticket_runs` row (dispatching→dispatched/failed, partial-failure isolated). **Never-merge guardrail VERIFIED by the orchestrator (independent source grep: zero merge invocations) + `never-merge.test.ts`.** 13 tests + 287 trpc pass. Migration `0010_ticket_runs.sql`. Commit `8838c819b`. (Returns `{ runs: TicketRunResult[] }`; B6 fills `pr_url`.)
- [x] B6 (2026-06-23) — DONE. Added an `onPullRequestLinked` first-detect hook to `PullRequestRuntimeManager` (fires once on the PR row's insert; swallows throws) → `reconcileTicketRunForPr` matches the PR head branch to a `ticket_runs.branch`, stamps `pr_url`, and writes prUrl + best-effort in-review status to the cloud task via `ctx.api.task.update` (→ Linear through the existing outbound `syncTask`; no cloud schema change). Multi-repo links every repo's PR to the one shared `taskId`. Idempotent + best-effort. 9 tests + 25 pull-requests + 22 ticket-runs + 229 integration pass; integrated typecheck + biome clean. Commit `eb4c55a9e`.
- [~] B7 (DEFERRED by design, 2026-06-23) — `mcpScope` forwarding is NOT contained host wiring: `mcpScope` is a cloud-only construct (jsonb column on `automations` + cloud automation router) already dropped at cloud dispatch, with ZERO host-side hook (0 matches in packages/host-service; `agentLaunchSchema`/`runAgentInWorkspace` carry no MCP field). Forwarding it needs a cloud `packages/trpc` schema/dispatch change (out of scope — no cloud schema change this wave) + new host MCP-config materialization + auth-scoping. Non-load-bearing: A1's loopback `/mcp/memory` already gives the agent memory pull and B6 already does the ticket/PR writeback host-side. Deferred per the time-box; no code changed. (See Future.)
...
**Wave 4 COMPLETE — 2026-06-23.** All required milestones (A1, A2, B1–B6) + the optional A3 landed and are green; B7 deferred-by-design (see Future). 16 commits on `claude/keen-euler-e9b3x8`.

**Against the Purpose — Part A:** the local memory **pull path is LIVE** — `createHostMemoryMcpServer` is bound to a loopback Streamable-HTTP `/mcp/memory` endpoint (manifest-discovered) and the `superset-memory` SKILL.md auto-regenerates on startup + memory mutations (A1); the stale `MemoryGraphView` docstring is fixed; and the waves-1/2/3 interactive acceptance is **recorded** (the A2 table above) — every machine-checkable layer green, the GUI-only steps a documented human handoff (no Playwright/Electron harness exists; not added, per the no-heavy-dep constraint). A3 (group/folder terminal respawn durability) shipped too.

**Against the Purpose — Part B:** the one-button pipeline ships end-to-end — "My tickets" assigned-to-me (B1) → optional context + a **no-cap** draft assembled from the ticket + live wave-3 memory (B2) → **edit + APPROVE, the single human gate** (B3) → repo scope, default one repo / exactly one confirm if multi (B4) → autonomous run: per-repo `workspaces.create` worktree+agent → commit/push → **PR opened, then STOP — never merged** (B5) → PR url + Linear in-review written back to the one ticket, multi-repo links every PR (B6).

**Decisions honored (verified):** exactly one human gate (context approval), enforced in code (B5 router maps a missing approved context → `PRECONDITION_FAILED`; the UI gates "Start autonomous run" on approval). Autonomous to **PR creation only — NEVER merge**, re-confirmed by an independent source grep + `never-merge.test.ts` (zero merge invocations anywhere; the only "merge" strings are the negated stop instruction + absence-assertions). One-repo default / ask-once multi-repo fan-out → PR-per-repo, all linked to the one ticket. **No token cap** on the assembled context (the model's window is the only bound; the host `memory.retrieve` cap of 20k/50 is a deliberate relevance aid, not a cap on the approved context). Precedence **developer-approved > project > global**, approved layer marked AUTHORITATIVE and placed LAST. Secrets **redacted** at persistence (B3 `saveApproved`) and prompt assembly (B5); the renderer draft defers redaction host-side (documented, since `redactAll` is Node-only). Approved context + run rows stored **host-side/local** (migrations `0008_ticket_context`, `0010_ticket_runs`) — **no cloud schema change**.

**Final acceptance (closing sweep):** `bun run typecheck` (turbo, whole tree) — **0 errors, 32 packages**; `packages/host-service` — **861 pass / 0 fail / 8 todo**; `apps/desktop` tasks — **36/0**; `packages/memory` — **127/0** (redaction 15/0); `bun run lint` — **exit 0** (biome @2.4.2 clean across 5033 files). Standing-gate caveats (environment, consistent with prior waves): whole-tree `bun test` is unusable (pty-daemon real-spawn crashes Bun) → per-package is the gate; the `rg`-based `check-git-ref-strings.sh` lint sub-check can't run locally (no ripgrep binary) and runs in CI — biome (the substantive gate) and the other two git-usage checks pass.

**Deferred / Future:** B7 `mcpScope` forwarding (cloud-only construct, non-load-bearing — see Future); a Playwright-electron GUI smoke driver (no harness exists; would need a Clerk test-auth bypass + native-picker IPC stub + a fixture host on a temp `SUPERSET_HOME_DIR`); the GUI-only steps of the waves-1/2/3 acceptance remain a human handoff with exact click-paths recorded above.
...
- **`mcpScope` forwarding to the autonomous agent (B7)** — deferred this wave (2026-06-23). It crosses into cloud `packages/trpc`/schema (forbidden this wave) and needs new host MCP-server-config materialization + auth-scoping, so it is not contained host wiring. It is non-load-bearing: the autonomous run already pulls memory via A1's loopback `/mcp/memory` and writes the ticket/PR back via B6. Future work once a cloud-side `mcpScope` dispatch path exists.
