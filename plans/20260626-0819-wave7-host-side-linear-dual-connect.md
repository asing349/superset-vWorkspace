...
- [x] M5 — Coexistence/precedence polish — **done 2026-06-26T11:05Z**: the dual connection is now first-class and the precedence rule is explicit in the UI, and M4's deferred local-ticket launch surface shipped. **Precedence-aware dual UI:** the local "This Mac" card gains an **Active** / **Dormant** indicator (active only when local-connected AND cloud-disconnected; dormant when a local token exists while cloud is active) driven by pure helpers (`linearLocalConnectionState.ts`: `deriveLinearLocalStatus` (expiry mapping), `formatLinearLocalStatusLabel`, `isLocalActiveSource`, `deriveLocalConnectAffordance` (cloud-precedence + host-readiness disabled/explainer)); the connect button stays disabled-with-explainer while cloud is active (both the host gate AND the UI). **Local ticket→PR launch (M4 deferral):** each `local:` row in the unified `IntegrationTickets` list gets a **Run → PR** toggle that opens a `LocalTicketRunPanel` — it reuses the cloud page's `TicketRunLauncher` + `useTicketContextApproval` **without touching the protected cloud page** (cross-route barrel import), wiring the approval gate keyed `(primaryProjectId, taskId)` where **`taskId` = `unifiedId` = `local:<issueId>`** so the host routes writeback to the local Linear source; context is a plain editable draft (`buildLocalTicketContextDraft`) since the integrations route has no `WorkspaceClientProvider`. Cloud tickets keep launching from the cloud task page (unchanged). **Expiry → one-button reconnect:** an expired local access token (pure `deriveLinearLocalStatus` → `"expired"`) surfaces a **Reconnect** button that runs the host's lazy PKCE `refresh` (which purges + falls back to a fresh connect on `invalid_grant`). **Disconnect:** a confirm dialog precedes `linear.auth.disconnect` (which already purges ONLY the local token — verified). **Per-source team filter:** unchanged from M3 (already local-only). **Docs:** `apps/desktop/docs/LINEAR_LOCAL_CONNECTION.md` (full setup + cloud-precedence) + a `DEVELOPMENT.md` note documenting `LINEAR_DESKTOP_CLIENT_ID` / `LINEAR_DESKTOP_REDIRECT_PORT` / redirect URI `http://127.0.0.1:<port>/callback`. Gates green: typecheck 29/29, `bun run lint` exit 0, `bun test packages/host-service` 1069 pass / 0 fail / 8 todo (unchanged — host surface untouched this milestone) + renderer `linearLocalConnectionState.test.ts` (16) & `buildLocalTicketContextDraft.test.ts` (3) pass. No host changes, NO new migration, cloud path unchanged.
...
- Observation (M5): the cloud page's `TicketRunLauncher` is NOT Electric-coupled and is fully reusable off the cloud route — its only providers (`useCollections`, `useLocalHostService`, `authClient`) are mounted at `_authenticated`, which the Settings → Integrations route already consumes (`IntegrationsSettings` itself calls both). So M4's "deeply Electric-coupled" worry was specifically about the **context builder** (`TicketContextBuilderInner` uses `workspaceTrpc.memory.retrieve`, which needs a `WorkspaceClientProvider`), NOT the launcher. M5 therefore reuses the launcher + the `useTicketContextApproval` hook verbatim via cross-route barrel imports (no edit to the protected cloud page) and substitutes a **plain editable draft** (`buildLocalTicketContextDraft`) for the memory-ranked builder — the approval gate (`ticketContext.getApproved/saveApproved`, keyed `(projectId, taskId)`) is provider-free and works as-is.
  Evidence: `TicketRunLauncher.tsx` imports only app-level providers; `useTicketContextApproval.ts` uses the vanilla `getHostServiceClientByUrl` (the task route has no `WorkspaceClientProvider` either). The local panel is the FIRST cross-route `tasks/$taskId/` import (only `routeTree.gen.ts` referenced that path before).

- Observation (M5): the local run needs `taskId === unifiedId === local:<issueId>` — using the same value for the approved-context key AND the writeback handle. Cloud differs (`taskId` = bare `tasks.id`, `unifiedId` = `cloud:<id>`), but for local there is no separate "bare" id to key on, so collapsing them is correct and the host accepts it (`dispatch.ts` parses `unifiedId` via `parseUnifiedTicketId` and stores it in `ticket_runs.taskId`; `local:` → the M4 direct-Linear writeback).

- Observation (M5): expiry is a **renderer-derived** state, not a host flag — `getConnection` already returns `expiresAt` (epoch ms) but no boolean. A pure `deriveLinearLocalStatus({ connection, now })` maps it to `disconnected|connected|expired`, and the "Reconnect" button reuses the **existing** host `linear.auth.refresh` (lazy PKCE refresh that self-purges on `invalid_grant`) — so no new host surface was needed for the expiry/re-auth handling, only UI state + a button.
...
- Decision (M5): surface the local-ticket launcher by **reusing the cloud page's `TicketRunLauncher` + `useTicketContextApproval` via cross-route barrel imports** (no edit to the protected cloud task page) inside a new `LocalTicketRunPanel`, and **substitute a plain editable context draft** (`buildLocalTicketContextDraft`) for the memory-ranked `TicketContextBuilder`. Rationale: the launcher and the approval hook are provider-free (only `_authenticated`-level providers, which the Integrations route already has); only the context *builder* needs `WorkspaceClientProvider` (`workspaceTrpc.memory.retrieve`), which the Integrations route lacks. A plain textarea draft keeps the panel browser-safe and dependency-light while still exercising the SAME approval gate + run contract. Alternatives rejected: promoting the launcher to a shared `components/` dir (would change the cloud page's import line — a touch of the protected page); standing up a `WorkspaceClientProvider` in Settings (heavy, and memory ranking isn't required to approve a local draft). Date/Author: 2026-06-26, M5 implementation.

- Decision (M5): for a local run, **`taskId` = `unifiedId` = `local:<issueId>`** (one value for both the approved-context key and the writeback handle). Rationale: unlike cloud (which has a distinct bare `tasks.id` to key approval on), local has only the Linear issue id, so collapsing the two is correct and the host already accepts it (`dispatch` parses `unifiedId` and stores it in `ticket_runs.taskId`; `local:` → M4's direct-Linear writeback). Date/Author: 2026-06-26, M5 implementation.

- Decision (M5): **expiry + re-auth are handled UI-side over the existing host surface** — a pure `deriveLinearLocalStatus({ connection, now })` maps the status's `expiresAt` to `disconnected|connected|expired`, and the "Reconnect" button calls the existing `linear.auth.refresh` (lazy PKCE refresh that self-purges + re-prompts on `invalid_grant`). No new host procedure, no persisted "needs-reconnect" flag. The cloud-precedence disabled-state + the active/dormant indicator are likewise pure helpers (`deriveLocalConnectAffordance` / `isLocalActiveSource`), unit-tested against fakes — the host gate remains the authority; the UI helpers only mirror it. Date/Author: 2026-06-26, M5 implementation.

- Decision (M5): **disconnect is confirm-gated in the UI** (an `alert(...)` dialog) before calling `linear.auth.disconnect`, which already purges ONLY the local token (verified: `LinearAuthService.disconnect` → `store.clear()`, no cloud round-trip). Rationale: a destructive, one-click token purge deserves a confirm; the cloud connection is explicitly called out as unaffected in the dialog copy. Date/Author: 2026-06-26, M5 implementation.
...
- **Writeback to the active source — delivered.** Cloud → `ctx.api.task.update` (byte-identical to wave-4); local → a NEW host-side **direct-Linear** writeback (PR-link comment + best-effort in-review state) with the M1 token, routed at the reconciler by the run's stored `unifiedId` (M4). Token never logged; no cloud round-trip for local.
- **Host-local encrypted tokens; no cloud schema change — held.** Two new host SQLite tables (`linear_local_auth` 0015, `linear_tickets` 0016) and nothing else; `packages/db`/`packages/trpc` cloud schema, the `integration_connections` table, the cloud `tasks` table, and the `apps/api` Linear routes are untouched (read/reused only). No new migration in M3–M5.

**What M5 added on top of M1–M4:** the dual connection is now first-class — independent cloud + local status with an explicit **Active / Dormant** indicator and the precedence explainer; **expiry → one-button Reconnect** (pure `deriveLinearLocalStatus` + the existing `linear.auth.refresh`); a **confirm before disconnect** (which purges only the local token); the deferred **local-ticket → PR** launch surface; and **from-source dev docs** for `LINEAR_DESKTOP_CLIENT_ID` / `LINEAR_DESKTOP_REDIRECT_PORT`.

**Retrospective — what went well:** cloud-precedence (mutual exclusivity) was the decisive simplification — every milestone collapsed a merge/arbitration problem into a single-source resolution. Reusing existing host precedents (OpenAI OAuth loopback, the wave-4 pipeline, the cloud `getConnection` status, the cloud Linear `ISSUES_QUERY`) meant almost no net-new infrastructure. Carrying the source as a `${source}:${sourceId}` `unifiedId` in the existing `ticket_runs.taskId` text column routed writeback with zero schema change. The injected-fakes discipline kept ~55 new host-service tests + the renderer helper tests live-API-free.

**Retrospective — friction / risks:** (1) M5's local launcher is the first cross-route `tasks/$taskId/` import — clean for now, but if a third consumer appears the launcher + approval hook should be promoted to a shared `components/`/`hooks/` dir. (2) The local context draft is plain (no memory ranking) because the Integrations route lacks `WorkspaceClientProvider`; a future unification could lift the memory-ranked builder behind a provider-agnostic seam. (3) The whole live path (PKCE browser handshake, real ticket fetch, real writeback, the precedence flip when cloud connects) **cannot be exercised in CI** — it requires a registered public Linear OAuth app (`LINEAR_DESKTOP_CLIENT_ID`) and, for the precedence flip, a real cloud Linear connection. Everything pure/host-side is unit-tested; the live integration is gated on a human running `bun dev` with a real client id.

**Cannot be verified without a live Linear OAuth app / connected provider:** the browser PKCE round-trip and loopback capture; a real viewer/workspace status card; real ticket polling + Refresh; a real local ticket→PR run with direct-Linear writeback; and the cloud→local precedence flip (needs a real cloud Linear connection to make cloud the active source). These are wired and type-safe end-to-end but await a live provider.

**Future (unchanged from below):** true simultaneous coexistence (merge+dedupe), PAT fallback, local webhooks, local→cloud writes, MCP-sourced tickets, and other providers via the same source-agnostic layer.
