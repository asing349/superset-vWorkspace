...
- (M6 renderer) The single-workspace `TerminalPane` is hard-coupled to `workspaceId`/`useWorkspace()` (WS-URL `workspaceId` query param, `listSessions.invalidate({workspaceId})`, `useTerminalInterruptClear`, `useOpenInExternalEditor`, the agent-binding `TerminalPaneIcon`). Per the M3 thin-wrapper pattern, M6 built a group-scoped `GroupTerminalPane` that REUSES the shared `terminalRuntimeRegistry`/`TerminalSearch`/`ScrollToBottomButton`/link-policies/`useTerminalAppearance`/drop-paste but drops every `workspaceId` coupling. **The WS connection attaches by `terminalId` alone — no `workspaceId` query param** — because group/agent sessions carry `originWorkspaceId = null`, so the host's `/terminal/:id` workspace-mismatch check (which only fires when BOTH a requested id AND a non-null record id exist) is skipped. The session's cwd + `SUPERSET_ROOTS` env are fixed at create time by the launcher, so WS-attach needs no addressing.

- (#10, cross-root quick-open) The single-workspace `QUICK_OPEN` quick-open store is hard-keyed to `{workspaceId}` (not reusable for groups), so a self-contained group quick-open was built (`useGroupFileSearch` + `GroupQuickOpen`). Fan-out uses `workspaceTrpc.useQueries` (one `searchFiles({groupId,rootId})` per existing root, concurrent over the single per-host connection), merged/deduped by `(rootId,absolutePath)` and interleaved by the host fuzzy `score`. The dashboard global palette uses `OPEN_COMMAND_PALETTE` (NOT `QUICK_OPEN`), and `QUICK_OPEN` is registered only inside single-workspace pages, so Cmd/Ctrl+P is free on group routes — the group hotkey is mounted only on the group route and never hijacks the global palette. `searchContent` has **zero** renderer consumers repo-wide (no v2 content-search panel), hence cross-root content search is deferred with a `TODO(group-content-search)`.

- (M6 renderer) **`filesystem.statPath` is `workspaceId`-only** (it was NOT extended to `{groupId,rootId}` like the listing/read procs in M1/#9), and host-service is off-limits to the renderer agent. So the terminal file-link `stat` handler routes via the targeting root's real `workspaceId` only for `kind:"workspace"` roots; folder roots and the combined agent terminal get non-interactive file links. This is acceptable because `statPath` resolves *absolute* paths host-wide (terminal output paths are typically absolute), and the explorer is the primary file-open surface. If interactive folder-root terminal links are wanted later, extend `statPath` to accept `{groupId,rootId}` (host change).

- (M6 renderer) `LinkAction` is `"pane" | "newTab" | "external"` (no `"current"`). Group terminals have no workspace-scoped external-editor target, so the file-link click maps every action to an in-app editor open (`newTab` → new tab, otherwise current). `LinkHoverInfo.kind` is `"file" | "url"` (no `"folder"`); directory-ness is `info.isDirectory`.

- (M6 renderer) Group terminal `onAfterClose` disposes via `terminalRuntimeRegistry.dispose(terminalId)` only — the single-workspace path also calls `killTerminalSessionSilently({terminalId, workspaceId})`, which is `workspaceId`-keyed and inapplicable to a workspace-less group/agent session. Disposing the renderer runtime closes the transport; the host PTY exits when its socket closes. (Background-terminal adoption for group sessions is a future nicety, out of M6 scope.)
