...
- [x] (2026-06-20) M5 — Combined-agent UX. `createAgentRoot` already re-prepares the synthetic root on every launch (confirmed; idempotent + per-group serialized from M2). `useCombinedAgentRoots` tracks the launch-time root signature `(rootId,rootPath)` and shows a relaunch nudge for a running agent (terminal pane with `rootId==null`) when the root set changed since launch (baseline self-adopts on rehydrate). Exists-only "N roots visible to the combined agent" hint (matches `prepare-agent-root` `presentRoots`) in `GroupAddTabMenu` + empty-pane state. Committed `e7241100a`, pushed — typecheck 28/28, lint clean.
